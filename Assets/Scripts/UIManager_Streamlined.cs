...
public class UIManager_Streamlined : MonoBehaviour
{
    ...
    private async void OnHostSavedGameClicked(SavedGameInfo gameInfo)
    {
        ...
        // PRESET GAME CHECK: If this is Dice Race, show customization panel first
        if (gameInfo.gameType == "Dice Race" || gameInfo.gameType == "DiceRace" || gameInfo.gameName == "Dice Race")
        {
            Debug.Log("[UIManager_Streamlined] Dice Race detected - showing customization panel");
            
            // Check if PresetUIManager exists
            if (PresetUIManager.Instance == null)
            {
                Debug.LogError("[UIManager_Streamlined] PresetUIManager not found! Falling back to standard hosting.");
                // Fall through to standard hosting
            }
            else
            {
                // Store the game info for later use after customization
                StoreGameInfoForCustomization(gameInfo);
                
                // Show customization panel
                PresetUIManager.Instance.ShowDiceRaceCustomization();
                
                // Hide saved games panel
                HideAllPanels();
                
                return; // Exit here - customization panel will handle hosting
            }
        }

        // Standard hosting flow (for non-preset games)
