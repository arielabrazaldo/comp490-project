...
public class CustomGameAnalyzer : MonoBehaviour
{
    ...
    private float CalculateMonopolyScore(GameRules rules)
    {
        ...
        if (!rules.separatePlayerBoards) { score += 0.5f; checks++; }  // Shared board (reduced weight)
        if (rules.canSeeEnemyTokens) { score += 0.5f; checks++; }      // All players visible (reduced weight)
        if (rules.canPurchaseProperties) { score += 1f; checks++; }    // Property buying - KEY feature
        if (rules.enablePropertyTrading) { score += 1f; checks++; }    // Property trading - KEY feature
        ...
        if (rules.enableRentCollection) { score += 1f; checks++; }     // Rent collection - KEY feature
    ...
    private float CalculateBattleshipsScore(GameRules rules)
    {
        ...
        if (rules.separatePlayerBoards) { score += 1f; checks++; }     // Separate boards - KEY feature
        if (!rules.canSeeEnemyTokens) { score += 1f; checks++; }       // Hidden enemy pieces - KEY feature
        if (rules.enableCombat) { score += 1f; checks++; }             // Combat mechanics - KEY feature
        if (rules.enableShipPlacement) { score += 1f; checks++; }      // Ship placement - KEY feature
        if (rules.tilesPerSide >= 8) { score += 0.3f; checks++; }      // Larger board (reduced weight)
        // Removed !enableCurrency check - too generic, causes false positives
    ...
    private float CalculateDiceRaceScore(GameRules rules)
    {
        ...
        if (!rules.enableCurrency) { score += 1f; checks++; }          // No currency
        if (!rules.separatePlayerBoards) { score += 1f; checks++; }    // Shared board
        if (rules.canSeeEnemyTokens) { score += 1f; checks++; }        // All players visible
        if (!rules.canPurchaseProperties) { score += 1f; checks++; }   // No properties - KEY differentiator
        if (!rules.enablePropertyTrading) { score += 1f; checks++; }   // No trading - KEY differentiator
        if (!rules.enableCombat) { score += 1f; checks++; }            // No combat
        if (!rules.enableRentCollection) { score += 1f; checks++; }    // No rent - KEY differentiator
